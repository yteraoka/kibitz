...
| 設定 | 既定 | 効き方 |
| --- | --- | --- |
| `KIBITZ_MAX_DELIVERIES` (Terraform: `max_delivery_attempts - 1`) | **4** | 配信回数がこれ以上になったら諦めて PR に通知し、ack する |
| サブスクリプションの `max_delivery_attempts` | **5** | ワーカーが ack も nack もできなかった場合の保険。超えると DLQ |

つまり**通常の失敗は合計 4 回 (初回 + リトライ 3 回) で打ち切り**、4 回目で PR に
「レビューに失敗しました」というコメントが付く。Pub/Sub 側の 5 回目が効くのは、
ワーカーがクラッシュした・OOM で落ちた・インスタンスが強制終了されたなど、
**ワーカーが返事をしないまま消えた場合**だけ。

バックオフは `minimum_backoff = 10s` から `maximum_backoff = 600s` の指数バックオフ
なので、4 回は**数分以内**に終わる。デプロイで直す前に打ち切られるのが普通だと思ってよい。

注意点:

- **配信回数は DLQ ポリシーがあるサブスクリプションでしか取得できない。**
  Pub/Sub は `DeliveryAttempt` を DLQ ポリシー設定時にしか付けないため、
  ポリシーが無いと `Deliveries = 0` になり、**上限判定が効かずメッセージ保持期間まで
  リトライし続ける**。Terraform 構成では必ず設定している
- **ロック競合 (`RetryAfter`) も配信回数を消費する。** 打ち切り判定は通らないが、
  Pub/Sub の `max_delivery_attempts` には数えられるので、長いレビューの後ろで
  待たされ続けたメッセージは DLQ に入りうる
- `worker.Permanent` は現時点でどこからも呼ばれていないので、恒久的エラーによる
  早期打ち切りは実質発生しない
