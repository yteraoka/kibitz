...
Issue の内容を読んでコードを書き、ブランチと draft PR を作るモード。
...
| `implement.enabled: true` | リポジトリの `.kibitz.yaml`（**デフォルトブランチ**、[ADR-0011](adr/0011-repository-settings-from-the-default-branch.md)） |
| Issue 上での明示コマンド (`/kibitz implement`) | PR 上では動かない。PR には既にブランチと差分がある |
| 指示者が `implement.allowed_actors` に含まれる | 同上。**空なら誰も許可されない** |
| `implement.paths_allow` が 1 つ以上ある | 同上。**空なら何も書けない** |
| `implement.commands_allow` が 1 つ以上ある | 同上。検証できない変更は PR にしない |

**見るのは指示者で、Issue を書いた人ではない。** 本文はデータとして扱う
([ADR-0010](adr/0010-data-and-instruction-positions.md))。

### 実行の順番

順番がそのまま設計になっている。**何かを費やす前に「やらない」と言える判定を
先に置き、PR ができるかどうかを決める 2 つの判定を push より前に置く。**

```
1. 既にこの Issue の PR があるか          → あればそれを返す (モデルを呼ばない)
2. デフォルトブランチをチェックアウト
3. 同名ブランチがリモートに無いか          → あれば上書きせずに報告
4. kibitz-implement エージェントを実行
5. git に「実際に何が変わったか」を聞く     → 変更が無ければ理由を報告
6. 変更パスを paths_allow と固定の拒否リストで照合
                                          → 範囲外があれば変更を全部破棄
7. commands_allow を資格情報の無い別ジョブで実行
                                          → 失敗したら PR を作らず出力を報告
8. commit → push → draft PR
```

5 が「git に聞く」なのが要点。エージェントの報告を信じるのではなく、
ワークスペースの実際の状態を見る。6 は 5 が返したすべてのパスを照合する。

> **エージェントに伝えた `paths_allow` は制約ではない。** 制約は 6 の照合。
> opencode の permission も同じ一覧で絞るが、それは安い一層目で、
> ルールを伝えられたモデルはルールではない。

範囲外のパスが 1 つでもあれば、**変更は全部破棄する。** 許可された分だけを残すと、
片方だけ適用された編集が残る — それはどちらでもないものになる。
...
ワーカーのプロセスは GitHub App の秘密鍵・GitLab / Azure DevOps のトークン・
Vertex AI の ADC・Firestore・Pub/Sub を持っており、テストファイルは
PR を開ける人が誰でもコードを置ける場所である。

### ビルドとテストが走る場所

`kibitz-runner` が、**役割を 1 つも持たないサービスアカウント**の Cloud Run ジョブで
走る。到達できるのは 1 つのバケットの 1 プレフィックスだけ。**敵対的なテストが
メタデータサーバーからトークンを取っても、そのトークンでできることが無い**
([ADR-0019](adr/0019-run-repository-code-in-a-credential-less-job.md))。

ランナーに渡すのは**ツリーとコマンドだけ**で、どのリポジトリか・誰が指示したか・
何のための変更かを知らない。知っても使い道が無く、他人のテストを走らせる
プロセスには何も教えないほうがよい。

gVisor / Firecracker は採らなかった。どちらも GKE を意味し、worker pool を前提にした
[ADR-0014](adr/0014-worker-pool-and-self-managed-scaling.md) に反する。
**ホストの分離は既に済んでいる** — Cloud Run はコンテナを gVisor 上で動かす。
残っていたのは kibitz 自身の資格情報だけで、それには別のプラットフォームではなく
**別の identity** が要る。

**検証する場所が設定されていなければ、実装モードは動かない。** 検証を省略して
PR を作ることはしない。

### 出力

- 常に **draft PR**。本文の冒頭に「AI が Issue から書いたもので、人はまだ読んでいない」
  ことと、元 Issue へのリンク、実行したコマンドとその結果を明記する
- 人間のレビューとマージを必須とする (自動マージはしない)
- **kibitz が作った PR に kibitz はレビューしない。** PR を開いたのは kibitz 自身の
  アカウントなので、既存のループ防止 (イベントの actor が自分なら捨てる) がそのまま効く。
  人がそのブランチに push した場合は、その人の変更としてレビューする

### 設定に関わらず編集できないもの

`paths_allow` に `**` と書いても、CI 設定・`.kibitz.yaml` と `.kibitz/**`・`AGENTS.md`・
依存定義ファイルとロックファイル・`*.pem` / `*.key` / `.env` は編集できない。
**これらを書き換えられると、以後の実行環境そのものを乗っ取られる。**
とくに `.kibitz.yaml` は「編集してよいパスの一覧」そのものなので、リポジトリ側から
上書きできる設定にすると意味を失う。だから設定ではなく固定。

依存の追加も同じ理由でここに入る。**依存を 1 行足すことは、次のビルドで他人の
コードを走らせること**で、それは人が決めることである。
...
- 1 回の検証の上限は `KIBITZ_IMPLEMENT_VERIFY_TIMEOUT` (既定 15 分)
- コマンドは最大 10 個、出力は末尾 16 KB まで
- モデルの費用は月次の予算 (`KIBITZ_REPO_BUDGETS`) に計上される。
  **検証が失敗しても費用は発生している**ので、PR ができなくても計上する

### 残っているもの

- GitLab / Azure DevOps の `forge.Writer` と Issue クライアント
  (Azure DevOps は Work Item と Git リポジトリの紐付け解決が追加で必要)
- `issue.assigned` — コマンドと同じ機構の 2 つ目の入口
...
egress を塞ぐ。**リポジトリのコードは実行しない** — ビルドもテストも走らせない。
